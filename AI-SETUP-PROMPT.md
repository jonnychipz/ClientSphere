...
3. If LIVE_FABRIC_ENABLED=yes, ensure the four Fabric Data Agents are published and collect their workspace/item IDs. Assign every delegated user or group **Foundry Agent Consumer** on the project plus direct access to the Fabric Data Agents and sources. Register a single-tenant Entra web application with the exact `/auth/fabric/callback`, delegated Azure AI Foundry `user_impersonation`, and required consent. Run scripts/configure-live-fabric.ps1 so it stores the five Fabric IDs, securely stores the client secret, creates direct Data Agent MCP connections during refresh, and avoids adding A2A/toolbox hops.
